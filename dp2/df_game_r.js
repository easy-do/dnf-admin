function arryToStr(arry) {
	...
	for (var i = 0; i < arry.length; i++) {
		str += String.fromCharCode(arry[i]);
	}
	return str;
}
...
function sendMessage(message){
	...
	ip_family.host = "dnf-admin.dnf_dnf";
	ip_family.port = 9999;
	var socket = Socket.connect(ip_family);
	socket.then(function (socketConnection) {
		socketConnection.setNoDelay(true);
		var buffer = new ArrayBuffer(message.length * 2);
		var bytes = new Uint8Array(buffer);
		for (var i = 0, len = message.length; i < len; i++) {
			bytes[i] = message.charCodeAt(i);
		}
		var op = socketConnection.output.write(bytes)
		var ipt = socketConnection.input.read(bytes)
		op.then(function (result) {
			console.log("op result:" + result);
		}).catch(function (error) {
			console.log('op fail1:' + error);
		});
		ipt.then(function (result) {
			console.log("ipt result:" + result);
		}).catch(function (error) {
			console.log('ipt fail1:' + error);
		});
	});
...
function recepMessage(){
	// var ip_family = new Object();
	// ip_family.family = "ipv4";
	// ip_family.host = "0.0.0.0";
	// ip_family.port = 5000;
	// var socket = Socket.listen(ip_family)
	// socket.then(function (result) {
	// 	console.log("recepMessage:" + result);
	// }).catch(function (error) {
